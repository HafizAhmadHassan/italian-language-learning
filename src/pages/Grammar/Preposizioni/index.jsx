...
import { preposizioniStages, preposizioniExercises, preposizioniFinalBands } from './preposizioniData';
import {
...
const STORAGE_KEY = 'preposizioniResults';
...
function SectionLearner({ section }) {
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-t border-gray-100 dark:border-italian-dark-border">
              ...
              <td className="px-4 py-2.5 font-semibold text-italian-blue">{row.example}</td>
            </tr>
          ))}
        </tbody>
      </table>
        ...
        <div key={i} className="rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {g.items.map((item, j) => (
              <span
                key={j}
                className="inline-flex px-3 py-1.5 rounded-lg bg-italian-blue/10 text-italian-blue text-sm font-bold border border-italian-blue/20"
              >
                {item}
              </span>
            ))}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gray-50 dark:bg-italian-dark-surface">
            ...
            <th className="text-left px-4 py-2.5 font-semibold text-italian-blue text-xs uppercase">Example</th>
          </tr>
        </thead>
        <tbody>
              ...
              className={`inline-flex px-3 py-1.5 rounded-lg text-xs font-semibold border ${
                i === 0
                  ...
                  : i === 1
                    ...
                    : 'bg-italian-blue/10 text-italian-blue border-italian-blue/20'
              }`}
            >
              {ex}
            </span>
  ...
  return (
    <div className="space-y-4">
      ...
      {section.note && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-italian-blue/10 border border-italian-blue/25">
          <MessageSquare size={16} className="text-italian-blue mt-0.5 shrink-0" />
...
function ReviewSection() {
  return (
    <div className="space-y-6">
      <h3 className="text-base font-heading font-bold text-italian-charcoal dark:text-white">Final Review — Preposizioni Reference</h3>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-blue uppercase tracking-wider mb-3">The Key Idea</h4>
        <p className="text-lg font-semibold text-italian-charcoal dark:text-white">
          Prepositions are <span className="text-italian-blue">short connecting words that show relationships</span> of place, time, and purpose — they always come before a noun.
        </p>
        <div className="flex flex-wrap gap-2 mt-3">
          {['di — of / from', 'a — to / at', 'da — from / by', 'in — in / into', 'con — with', 'su — on / about', 'per — for / through', 'tra / fra — between'].map((p) => (
            <span key={p} className="inline-flex px-3 py-1.5 rounded-lg bg-italian-blue/10 text-italian-blue text-xs font-bold border border-italian-blue/20">{p}</span>
          ))}
        </div>
      </div>

      <div className="overflow-hidden rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card">
        <h4 className="text-sm font-bold text-italian-blue uppercase tracking-wider p-5 pb-3">The Core Four</h4>
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 dark:bg-italian-dark-surface">
              <th className="text-left px-5 py-2.5 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">Preposition</th>
              <th className="text-left px-5 py-2.5 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">Main uses</th>
              <th className="text-left px-5 py-2.5 font-semibold text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">Examples</th>
            </tr>
          </thead>
          <tbody>
            {[
              { prep: 'di', use: 'Belonging, origin, about', examples: 'Il libro è di Marco · Vengo di... · Parlo di matematica' },
              { prep: 'a', use: 'Destination, location, at time', examples: 'Vado a Roma · Abito a Milano · a mezzogiorno' },
              { prep: 'da', use: 'Origin, by (agent), at someone\'s place', examples: 'Vengo da Roma · fatto da Marco · vado da Maria' },
              { prep: 'in', use: 'Countries, regions, inside, means', examples: 'Vivo in Italia · sono in ufficio · vengo in treno' },
            ].map((row, i) => (
              <tr key={i} className="border-t border-gray-100 dark:border-italian-dark-border">
                <td className="px-5 py-3 font-bold text-italian-blue">{row.prep}</td>
                <td className="px-5 py-3 text-gray-600 dark:text-gray-400 font-medium">{row.use}</td>
                <td className="px-5 py-3 text-gray-700 dark:text-gray-300">{row.examples}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-blue uppercase tracking-wider mb-3">A or IN? (cities vs countries)</h4>
          <div className="space-y-3">
            <div className="p-3 rounded-xl bg-italian-blue/5 border border-italian-blue/20">
              <p className="text-xs font-bold text-italian-blue uppercase tracking-wider mb-1.5">a + cities / small places</p>
              <div className="space-y-1">
                {['a Roma', 'a Milano', 'a casa', 'a scuola', 'a lavoro'].map((u) => (
                  <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
                ))}
              </div>
            </div>
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">in + countries / regions / buildings</p>
              <div className="space-y-1">
                {['in Italia', 'in Toscana', 'in ufficio', 'in cucina', 'in banca'].map((u) => (
                  <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
                ))}
              </div>
            </div>
        ...
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-blue uppercase tracking-wider mb-3">DI vs DA</h4>
          <div className="space-y-3">
            <div className="p-3 rounded-xl bg-italian-blue/5 border border-italian-blue/20">
              <p className="text-xs font-bold text-italian-blue uppercase tracking-wider mb-1.5">di — belonging / what it is</p>
              <div className="space-y-1">
                {['Il libro è di Marco.', 'La macchina di Anna è rossa.', 'Una tazza di caffè.'].map((u) => (
                  <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
                ))}
              </div>
            </div>
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">da — origin / movement away</p>
              <div className="space-y-1">
                {['Vengo da Roma.', 'Esco da casa.', 'Questo panino è dal bar.'].map((u) => (
                  <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
                ))}
              </div>
            </div>
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-blue uppercase tracking-wider mb-3">Time Expressions</h4>
        <div className="flex flex-wrap gap-2">
          {['alle otto — at eight', 'a mezzogiorno — at noon', 'da due anni — for two years', 'per due ore — for two hours', 'tra due giorni — in two days', 'fra un\'ora — in an hour', 'di mattina — in the morning', 'di notte — at night', 'per l\'estate — by/for the summer'].map((p) => (
            <span key={p} className="inline-flex px-3 py-1.5 rounded-lg bg-italian-blue/10 text-italian-blue text-xs font-bold border border-italian-blue/20">{p}</span>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-blue uppercase tracking-wider mb-3">Articulated Prepositions (il, lo, la, i, gli, le)</h4>
        <div className="space-y-3">
          {[
            { prep: 'a', forms: ['al', 'allo', 'alla', 'ai', 'agli', 'alle'] },
            { prep: 'di', forms: ['del', 'dello', 'della', 'dei', 'degli', 'delle'] },
            { prep: 'da', forms: ['dal', 'dallo', 'dalla', 'dai', 'dagli', 'dalle'] },
            { prep: 'in', forms: ['nel', 'nello', 'nella', 'nei', 'negli', 'nelle'] },
            { prep: 'su', forms: ['sul', 'sullo', 'sulla', 'sui', 'sugli', 'sulle'] },
          ].map(({ prep, forms }) => (
            <div key={prep} className="flex items-center gap-2 flex-wrap">
              <span className="w-8 shrink-0 text-sm font-bold text-italian-blue">{prep}</span>
              <div className="flex flex-wrap gap-1.5">
                {forms.map((f) => (
                  <span key={f} className="inline-flex px-2.5 py-1 rounded-lg bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border text-xs font-bold text-italian-charcoal dark:text-gray-100">{f}</span>
                ))}
              </div>
            </div>
          ))}
          <p className="text-xs text-gray-400 mt-2">per and tra/fra never combine: always <span className="font-semibold text-italian-charcoal dark:text-gray-200">per il</span>, <span className="font-semibold text-italian-charcoal dark:text-gray-200">tra i</span>.</p>
        </div>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-blue uppercase tracking-wider mb-3">Common Fixed Expressions</h4>
        <div className="flex flex-wrap gap-2">
          {['andare a cavallo', 'andare in macchina', 'andare in treno', 'andare a piedi', 'soffrire di...', 'venire da...', 'credere in...', 'parlare con...', 'grazie per...', 'votare per...'].map((p) => (
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-blue uppercase tracking-wider mb-3">Thinking Like a Native</h4>
        <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
          Italian uses <span className="font-semibold text-italian-charcoal dark:text-gray-200">di/a</span> where English often uses nothing: <em>Sono di Roma</em> (I am from Rome), <em>Andiamo a casa</em> (Let's go home). And it uses the <span className="font-semibold text-italian-charcoal dark:text-gray-200">articulated form</span> when a definite article is needed: <em>il libro sul tavolo</em> (the book on the table), <em>vado al cinema</em> (I go to the cinema).
        </p>
      </div>
    </div>
  );
...
function ResultsSummary({ results, onReset }) {
  ...
  const total = preposizioniExercises.length;
  const accuracy = Math.round((completed / total) * 100);

  const bands = preposizioniFinalBands;
  const band = bands.find((b) => accuracy >= b.min) || bands[bands.length - 1];

  const weakLevels = {};
  preposizioniExercises.forEach((e) => {
  ...
  const reviewMap = {
    'Level 1': 'Review what a preposition is and the basic meanings of di, a, da, in, con, su, per, tra/fra.',
    'Level 2': 'Review the core four — di (belonging), a (destination), da (origin), in (location) — and their key example sentences.',
    'Level 3': 'Review con, su, per, and tra/fra: with, on/about, purpose, and between/in-a-time.',
    'Level 4': 'Review how context picks the right preposition, time expressions (alle, da, per, tra), and a vs in for places and means of transport.',
    'Level 5': 'Review the articulated forms (al, del, dal, nel, sul...) and which prepositions never combine (per, tra/fra).',
    'Level 6': 'Practice building sentences from words and translating full sentences that mix several prepositions.',
    'Level 7': 'Re-read the stories — pay attention to how prepositions link place, time, and purpose naturally.',
  };
          ...
          <div className="space-y-3">
            {Object.entries(weakLevels).map(([lvl, count]) => (
              <div key={lvl} className="flex items-start gap-3">
                <span className="px-2 py-0.5 rounded-md bg-italian-blue/10 text-italian-blue text-xs font-bold border border-italian-blue/20 shrink-0 mt-0.5">
                  {lvl}
                </span>
                <div>
...
const allExercisesDone = (results) =>
  preposizioniExercises.every((e) => results[e.id]?.correct);

const STAGE_ORDER = ['concept', 'core', 'others', 'choose', 'exercises', 'stories', 'final'];
const LEARNING_STAGES = ['concept', 'core', 'others', 'choose'];
const STORY_EXERCISE_START = preposizioniExercises.findIndex((e) => e.type === 'story');

export default function PreposizioniLessonPage() {
  ...
  const completedCount = useMemo(
    () => preposizioniExercises.filter((e) => results[e.id]?.correct).length,
    [results]
  );
  ...
  const exerciseStart = useMemo(() => ({
    ...
    final: preposizioniExercises.length - 1,
  }), []);

  const currentEx = preposizioniExercises[currentExercise];
  const isFinalStage = activeStage === 'final';
  const showSummary = isFinalStage && allExercisesDone(results);

  const goToExercise = (idx) => {
    const clamped = Math.max(0, Math.min(preposizioniExercises.length - 1, idx));
    setCurrentExercise(clamped);
    const ex = preposizioniExercises[clamped];
  ...
  const handleStageClick = (stage) => {
    ...
    if (stage === 'final') setCurrentExercise(preposizioniExercises.length - 1);
  };

  const globalProgress = Math.round((completedCount / preposizioniExercises.length) * 100);
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-italian-blue/10 flex items-center justify-center">
              <BookOpen size={22} className="text-italian-blue" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="flex-1 h-1.5 bg-gray-100 dark:bg-italian-dark-surface rounded-full overflow-hidden">
              <motion.div
                ...
                className="h-full bg-italian-blue rounded-full"
              />
            </div>
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 shrink-0">
              {completedCount}/{preposizioniExercises.length} exercises
            </span>
          </div>
        </motion.div>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0">
          {[
            { id: 'concept', icon: BookOpen, label: '1. What are Prepositions?' },
            { id: 'core', icon: Layers, label: '2. Di, A, Da, In' },
            { id: 'others', icon: Grid3x3, label: '3. Con, Su, Per, Tra/Fra' },
            { id: 'choose', icon: Type, label: '4. Choices, Time & Articles' },
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 shrink-0 ${
                activeStage === id
                  ? 'bg-italian-blue text-white shadow-md shadow-italian-blue/20'
          ...
          {LEARNING_STAGES.includes(activeStage) && (
            ...
            >
              <h2 className="text-lg font-heading font-bold text-italian-charcoal dark:text-white mb-1">
                {preposizioniStages[STAGE_ORDER.indexOf(activeStage)]?.title}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                {preposizioniStages[STAGE_ORDER.indexOf(activeStage)]?.subtitle}
              </p>
              <div className="space-y-6">
                {preposizioniStages[STAGE_ORDER.indexOf(activeStage)]?.sections.map((section, idx) => (
                  <div
                    key={idx}
              ...
              <div className="flex justify-end mt-6">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-italian-blue text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-blue/20 transition-all duration-200"
                >
                  Continue to {preposizioniStages[STAGE_ORDER.indexOf(activeStage) + 1]?.title} <ArrowRight size={16} />
                </button>
              </div>
            </motion.div>
          )}
                ...
                <div className="space-y-6">
                          ...
                          className={`w-9 h-9 shrink-0 rounded-lg text-xs font-bold transition-all duration-200 flex items-center justify-center ${
                            i === currentExercise
                              ...
                              : results[e.id]?.correct
                                ? 'bg-italian-blue/10 text-italian-blue border border-italian-blue/20'
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-gray-400 dark:text-gray-500">
                      Exercise {currentEx.id} / {preposizioniExercises.length}
                    </span>
                    <button
                      onClick={() => goToExercise(currentExercise + 1)}
                      disabled={currentExercise === preposizioniExercises.length - 1}
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-italian-blue text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-blue/20 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
                    >
