...
import { congiuntivoStages, congiuntivoExercises, congiuntivoFinalBands } from './congiuntivoData';
import {
...
const STORAGE_KEY = 'congiuntivoResults';
...
function SectionLearner({ section }) {
  ...
  const renderExamples = (examples) => (
    <div className="space-y-2">
      {examples.map((ex, i) => (
        ...
        >
          <span className="text-sm font-semibold text-italian-rose whitespace-nowrap">
            {ex.italian}
          </span>
        ...
        <tbody>
          {conv.rows.map((row, i) => (
            <tr key={i} className="border-t border-gray-100 dark:border-italian-dark-border">
              ...
              <td className="px-4 py-2.5 font-semibold text-italian-rose">{row.ending}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  const renderTimeline = (tl) => (
    <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
      <div className="flex items-center justify-between text-xs font-bold text-gray-400 dark:text-gray-500 mb-3">
        <span>{tl.left}</span>
        <span className="text-italian-rose">{tl.center}</span>
        <span>{tl.right}</span>
      </div>
      <div className="relative h-1 bg-gray-200 dark:bg-italian-dark-border rounded-full mb-4">
        ...
        <div className="absolute left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-italian-rose ring-2 ring-italian-rose/30" />
          ...
          {tl.examples.map((ex, i) => (
            <span
              ...
              className={`inline-flex px-3 py-1.5 rounded-lg text-xs font-semibold border ${
                i === 1
                  ? 'bg-italian-rose/10 text-italian-rose border-italian-rose/20'
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        {focus.breakdown.map((b, i) => (
          ...
          >
            <p className="text-base font-bold text-italian-rose">{b.part}</p>
  ...
  return (
    <div className="space-y-4">
      ...
      {section.patterns && (
        <div className="flex flex-wrap gap-2">
          {section.patterns.map((p, i) => (
            <span
              key={i}
              className="inline-flex px-3 py-1.5 rounded-lg bg-italian-rose/10 text-italian-rose text-sm font-bold border border-italian-rose/20"
            >
              {p}
            </span>
          ))}
      ...
      {section.note && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-italian-rose/10 border border-italian-rose/25">
          <MessageSquare size={16} className="text-italian-rose mt-0.5 shrink-0" />
...
function ReviewSection() {
  return (
    <div className="space-y-6">
      <h3 className="text-base font-heading font-bold text-italian-charcoal dark:text-white">Final Review — Congiuntivo Reference</h3>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-rose uppercase tracking-wider mb-3">The Key Idea</h4>
        <p className="text-lg font-semibold text-italian-charcoal dark:text-white">
          Congiuntivo = <span className="text-italian-rose">doubt, opinion, emotion, and hypothesis</span> — the subjective mood that appears after a trigger + <span className="text-italian-rose">che</span>.
        </p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-rose uppercase tracking-wider mb-3">Present Subjunctive Endings</h4>
        <div className="flex flex-wrap gap-2">
          {['-ARE: parli, parli, parli, parliamo, parliate, parlino', '-ERE: prenda, prenda, prenda, prendiamo, prendiate, prendano', '-IRE: parta / finisca, … finiamo, finiate, partano / finiscano', 'essere: sia, sia, sia, siamo, siate, siano', 'avere: abbia, abbia, abbia, abbiamo, abbiate, abbiano', 'modals: debba, possa, voglia'].map((p) => (
            <span key={p} className="inline-flex px-3 py-1.5 rounded-lg bg-italian-rose/10 text-italian-rose text-xs font-bold border border-italian-rose/20">{p}</span>
          ))}
        </div>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-rose uppercase tracking-wider mb-3">Irregular &amp; Auxiliary Forms</h4>
        <div className="flex flex-wrap gap-2">
          {['dare → dia', 'stare → stia', 'fare → faccia', 'andare → vada', 'Passato: abbia/sia + participle', 'Imperfetto: -ASSI/-ESSI/-ISSI', 'Imperfetto: fossi / avessi', 'Trapassato: avessi/fossi + participle'].map((p) => (
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-rose uppercase tracking-wider mb-3">Trigger Families (all take the congiuntivo)</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-italian-rose/5 border border-italian-rose/20">
            <p className="text-xs font-bold text-italian-rose uppercase tracking-wider mb-2">Opinions &amp; thoughts</p>
            <div className="space-y-1">
              {['Credo che…', 'Penso che…', 'Sembra che…', 'Non credo che…'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Impersonal &amp; necessity</p>
            <div className="space-y-1">
              {['È possibile che…', 'È necessario che…', 'È importante che…', 'Bisogna che…'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Emotions</p>
            <div className="space-y-1">
              {['Mi dispiace che…', 'Sono felice che…', 'Che peccato che…'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
          <div className="p-4 rounded-xl bg-italian-rose/5 border border-italian-rose/20">
            <p className="text-xs font-bold text-italian-rose uppercase tracking-wider mb-2">Wishes, desires &amp; doubt</p>
            <div className="space-y-1">
              {['Voglio che…', 'Spero che…', 'Preferisco che…', 'Non so se sia…'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
        </div>
        <p className="text-xs text-gray-400 mt-3">Careful: certainty expressions (so che, è chiaro che, sono sicuro che) take the INDICATIVO.</p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-rose uppercase tracking-wider mb-3">Se-clauses (periodo ipotetico)</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Type 2 — Hypothetical</p>
            <div className="space-y-1">
              {['Se avessi tempo, verrei.', 'Se fossi in te, non lo farei.', 'Se potessi, partirei domani.'].map((f) => (
                <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
              ))}
            </div>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Type 3 — Impossible past</p>
            <div className="space-y-1">
              {['Se avessi saputo, sarei venuto.', 'Se avessi studiato, avrei passato.', 'Se fossi partito prima, non avrei perso il treno.'].map((f) => (
                <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
              ))}
            </div>
          </div>
        </div>
        <p className="text-xs text-gray-400 mt-3">Type 1 (real) stays in the indicativo: Se piove, prendo l\'ombrello.</p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-rose uppercase tracking-wider mb-3">Golden Rule</h4>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Look for the <span className="font-semibold text-italian-charcoal dark:text-white">trigger + che</span>. If the main clause expresses an <span className="font-semibold text-italian-charcoal dark:text-white">opinion, emotion, wish, doubt, or impersonal value</span>, the next verb goes in the congiuntivo. If it states a <span className="font-semibold text-italian-charcoal dark:text-white">certainty or fact</span>, keep the indicativo.
        </p>
      </div>
    </div>
  );
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="px-2.5 py-1 rounded-lg bg-italian-rose/10 text-italian-rose text-xs font-bold border border-italian-rose/20">
          ...
          {isDone && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-italian-rose/10 text-italian-rose text-xs font-bold">
...
function ResultsSummary({ results, onReset }) {
  ...
  const reviewMap = {
    'Level 1': 'Review what the congiuntivo is — the mood of doubt, opinion, wish, and emotion, introduced by "che".',
    'Level 2': 'Review the present subjunctive endings: -ARE → -i/-i/-i/-iamo/-iate/-ino and -ERE/-IRE → -a/-a/-a/-iamo/-iate/-ano.',
    'Level 3': 'Review essere (sia), avere (abbia), dare (dia), stare (stia), fare (faccia) and the modals (debba, possa, voglia).',
    'Level 4': 'Review the trigger expressions: opinions (penso che), impersonal (è possibile che, bisogna che), emotions and wishes (mi dispiace che, voglio che, spero che).',
    'Level 5': 'Review the passato congiuntivo: abbia/sia + past participle, with the auxiliary chosen like the passato prossimo.',
    'Level 6': 'Review the imperfetto congiuntivo (-assi/-essi/-issi), fosse, avessi, potessi, and its role in hypothetical se-clauses.',
    'Level 7': 'Review the trapassato congiuntivo (avessi/fossi + participle) and the periodo ipotetico type 3.',
    'Level 8': 'Practice choosing congiuntivo vs indicativo: certainties (so che, sono sicuro che, è chiaro che) take the indicativo.',
    'Level 9': 'Re-read the dialogues — notice the trigger expressions and which tense of the congiuntivo follows each one.',
  };
  ...
  return (
    <div className="space-y-6">
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="rounded-2xl border border-italian-rose/30 bg-italian-rose/5 p-6 text-center"
      >
        <Trophy className="w-10 h-10 text-italian-rose mx-auto mb-2" />
        <p className="text-3xl font-heading font-bold text-italian-rose">{accuracy}%</p>
          ...
          <div className="space-y-3">
            {Object.entries(weakLevels).map(([lvl, count]) => (
              <div key={lvl} className="flex items-start gap-3">
                <span className="px-2 py-0.5 rounded-md bg-italian-rose/10 text-italian-rose text-xs font-bold border border-italian-rose/20 shrink-0 mt-0.5">
                  {lvl}
                </span>
                <div>
...
const allExercisesDone = (results) =>
  congiuntivoExercises.every((e) => results[e.id]?.correct);
...
const STORY_EXERCISE_START = congiuntivoExercises.findIndex((e) => e.type === 'story');

export default function CongiuntivoLessonPage() {
  ...
  const completedCount = useMemo(
    () => congiuntivoExercises.filter((e) => results[e.id]?.correct).length,
    [results]
  );
  ...
  const exerciseStart = useMemo(() => ({
    ...
    final: congiuntivoExercises.length - 1,
  }), []);

  const currentEx = congiuntivoExercises[currentExercise];
  const isFinalStage = activeStage === 'final';
  const showSummary = isFinalStage && allExercisesDone(results);

  const goToExercise = (idx) => {
    const clamped = Math.max(0, Math.min(congiuntivoExercises.length - 1, idx));
    setCurrentExercise(clamped);
    const ex = congiuntivoExercises[clamped];
  ...
  const handleStageClick = (stage) => {
    ...
    if (stage === 'final') setCurrentExercise(congiuntivoExercises.length - 1);
  };

  const globalProgress = Math.round((completedCount / congiuntivoExercises.length) * 100);
    ...
    <div className="min-h-full pb-24 md:pb-8">
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mb-6">
          <Link to="/grammar" className="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-italian-rose transition-colors mb-4">
            <ArrowLeft size={16} /> Back to Grammar
          </Link>
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-italian-rose/10 flex items-center justify-center">
              <BookOpen size={22} className="text-italian-rose" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                Opinions, wishes, emotions, and hypotheses — the mood of subjectivity
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="flex-1 h-1.5 bg-gray-100 dark:bg-italian-dark-surface rounded-full overflow-hidden">
              <motion.div
                ...
                className="h-full bg-italian-rose rounded-full"
              />
            </div>
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 shrink-0">
              {completedCount}/{congiuntivoExercises.length} exercises
            </span>
          </div>
        </motion.div>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0">
          {[
            { id: 'concept', icon: BookOpen, label: '1. What is the Congiuntivo?' },
            { id: 'formation', icon: Layers, label: '2. Forming the Present' },
            { id: 'irregulars', icon: Grid3x3, label: '3. Passato & Imperfetto' },
            { id: 'usage', icon: Type, label: '4. When to Use It' },
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 shrink-0 ${
                activeStage === id
                  ? 'bg-italian-rose text-white shadow-md shadow-italian-rose/20'
          ...
          {LEARNING_STAGES.includes(activeStage) && (
            ...
            >
              <h2 className="text-lg font-heading font-bold text-italian-charcoal dark:text-white mb-1">
                {congiuntivoStages[STAGE_ORDER.indexOf(activeStage)]?.title}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                {congiuntivoStages[STAGE_ORDER.indexOf(activeStage)]?.subtitle}
              </p>
              <div className="space-y-6">
                {congiuntivoStages[STAGE_ORDER.indexOf(activeStage)]?.sections.map((section, idx) => (
                  <div
                    key={idx}
              ...
              <div className="flex justify-end mt-6">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-italian-rose text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-rose/20 transition-all duration-200"
                >
                  Continue to {congiuntivoStages[STAGE_ORDER.indexOf(activeStage) + 1]?.title} <ArrowRight size={16} />
                </button>
              </div>
            </motion.div>
          )}
                ...
                <div className="space-y-6">
                          ...
                          className={`w-9 h-9 shrink-0 rounded-lg text-xs font-bold transition-all duration-200 flex items-center justify-center ${
                            i === currentExercise
                              ...
                              : results[e.id]?.correct
                                ? 'bg-italian-rose/10 text-italian-rose border border-italian-rose/20'
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-gray-400 dark:text-gray-500">
                      Exercise {currentEx.id} / {congiuntivoExercises.length}
                    </span>
                    <button
                      onClick={() => goToExercise(currentExercise + 1)}
                      disabled={currentExercise === congiuntivoExercises.length - 1}
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-italian-rose text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-rose/20 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
                    >
