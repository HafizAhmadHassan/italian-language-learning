...
import { imperfettoStages, imperfettoExercises, imperfettoFinalBands } from './imperfettoData';
import {
...
const STORAGE_KEY = 'imperfettoResults';
...
function ReviewSection() {
  return (
    <div className="space-y-6">
      <h3 className="text-base font-heading font-bold text-italian-charcoal dark:text-white">Final Review — Imperfetto Reference</h3>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-purple uppercase tracking-wider mb-3">The Key Idea</h4>
        <p className="text-lg font-semibold text-italian-charcoal dark:text-white">
          Imperfetto = <span className="text-italian-purple">what was happening, used to happen, or was like</span> in the past.
        </p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-purple uppercase tracking-wider mb-3">Main Uses</h4>
        <div className="space-y-2">
          {[
            { use: 'Habit / repetition', example: 'Ogni giorno studiavo.' },
            { use: 'Description', example: 'La casa era grande.' },
            { use: 'Background', example: 'Pioveva.' },
            { use: 'Age', example: 'Avevo dieci anni.' },
            { use: 'Time', example: 'Erano le otto.' },
            { use: 'Ongoing action', example: 'Mentre studiavo, ascoltavo la musica.' },
          ].map(({ use, example }) => (
            <div key={use} className="flex flex-wrap gap-2 items-baseline p-3 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
              <span className="text-xs font-bold text-italian-purple uppercase tracking-wider">{use}</span>
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {['-ARE: -avo, -avi, -ava, -avamo, -avate, -avano', '-ERE: -evo, -evi, -eva, -evamo, -evate, -evano', '-IRE: -ivo, -ivi, -iva, -ivamo, -ivate, -ivano'].map((p) => (
            <span key={p} className="inline-flex px-3 py-1.5 rounded-lg bg-italian-purple/10 text-italian-purple text-xs font-bold border border-italian-purple/20">{p}</span>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          ...
          <div className="space-y-1.5">
            {['ero', 'eri', 'era', 'eravamo', 'eravate', 'erano'].map((f) => (
        ...
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          ...
          <div className="space-y-1.5">
            {['avevo', 'avevi', 'aveva', 'avevamo', 'avevate', 'avevano'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
        </div>
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-green uppercase tracking-wider mb-3">Fare</h4>
          <div className="space-y-1.5">
            {['facevo', 'facevi', 'faceva', 'facevamo', 'facevate', 'facevano'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-gold uppercase tracking-wider mb-3">Useful Signal Words</h4>
        <div className="flex flex-wrap gap-2">
          {['sempre', 'spesso', 'di solito', 'ogni giorno', 'ogni settimana', 'ogni estate', 'da bambino/a', 'quando ero piccolo/a', 'mentre'].map((irr) => (
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-purple uppercase tracking-wider mb-3">Imperfetto vs Passato Prossimo</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-italian-purple/5 border border-italian-purple/20">
            <p className="text-xs font-bold text-italian-purple uppercase tracking-wider mb-2">Imperfetto</p>
            <div className="space-y-1">
              {['Habit', 'Description', 'Background', 'Age', 'Time', 'Weather', 'Ongoing action'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Passato Prossimo</p>
            <div className="space-y-1">
              {['Completed event', 'One finished action'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
...
function ResultsSummary({ results, onReset }) {
  ...
  const reviewMap = {
    'Level 1': 'Review what the imperfetto is and its main uses: habits, descriptions, background, age, time.',
    'Level 2': 'Review the three regular ending patterns: -ARE → -avo/-avi/-ava, -ERE → -evo/-evi/-eva, -IRE → -ivo/-ivi/-iva.',
    'Level 3': 'Review the irregular forms of essere, avere, and fare.',
    'Level 4': 'Review when to use the imperfetto vs the passato prossimo, and practice signal words.',
    'Level 5': 'Practice building sentences from words and changing subjects.',
    'Level 6': 'Re-read the dialogues and stories — pay attention to the meaning of each form.',
    'Level 7': 'Re-read the stories and try producing your own sentences about your childhood or daily routine.',
  };
          ...
          <div className="space-y-3">
            {Object.entries(weakLevels).map(([lvl, count]) => (
              <div key={lvl} className="flex items-start gap-3">
                <span className="px-2 py-0.5 rounded-md bg-italian-purple/10 text-italian-purple text-xs font-bold border border-italian-purple/20 shrink-0 mt-0.5">
                  {lvl}
                </span>
                <div>
...
const allExercisesDone = (results) =>
  imperfettoExercises.every((e) => results[e.id]?.correct);

const STAGE_ORDER = ['concept', 'formation', 'irregulars', 'timecompare', 'exercises', 'stories', 'final'];
const LEARNING_STAGES = ['concept', 'formation', 'irregulars', 'timecompare'];
const STORY_EXERCISE_START = imperfettoExercises.findIndex((e) => e.type === 'story');

export default function ImperfettoLessonPage() {
  ...
  const completedCount = useMemo(
    () => imperfettoExercises.filter((e) => results[e.id]?.correct).length,
    [results]
  );
  ...
  const exerciseStart = useMemo(() => ({
    ...
    final: imperfettoExercises.length - 1,
  }), []);

  const currentEx = imperfettoExercises[currentExercise];
  const isFinalStage = activeStage === 'final';
  const showSummary = isFinalStage && allExercisesDone(results);

  const goToExercise = (idx) => {
    const clamped = Math.max(0, Math.min(imperfettoExercises.length - 1, idx));
    setCurrentExercise(clamped);
    const ex = imperfettoExercises[clamped];
  ...
  const handleStageClick = (stage) => {
    ...
    if (stage === 'final') setCurrentExercise(imperfettoExercises.length - 1);
  };

  const globalProgress = Math.round((completedCount / imperfettoExercises.length) * 100);
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-italian-purple/10 flex items-center justify-center">
              <BookOpen size={22} className="text-italian-purple" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                Imperfetto — habits, descriptions, background, and irregulars
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="flex-1 h-1.5 bg-gray-100 dark:bg-italian-dark-surface rounded-full overflow-hidden">
              <motion.div
                ...
                className="h-full bg-italian-purple rounded-full"
              />
            </div>
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 shrink-0">
              {completedCount}/{imperfettoExercises.length} exercises
            </span>
          </div>
        </motion.div>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0">
          {[
            { id: 'concept', icon: BookOpen, label: '1. What is the Imperfetto?' },
            { id: 'formation', icon: Layers, label: '2. Forming the Imperfetto' },
            { id: 'irregulars', icon: Grid3x3, label: '3. Essere, Avere, Fare' },
            { id: 'timecompare', icon: Type, label: '4. Time, Age & Comparing' },
          ...
          {LEARNING_STAGES.includes(activeStage) && (
            ...
            >
              <h2 className="text-lg font-heading font-bold text-italian-charcoal dark:text-white mb-1">
                {imperfettoStages[STAGE_ORDER.indexOf(activeStage)]?.title}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                {imperfettoStages[STAGE_ORDER.indexOf(activeStage)]?.subtitle}
              </p>
              <div className="space-y-6">
                {imperfettoStages[STAGE_ORDER.indexOf(activeStage)]?.sections.map((section, idx) => (
                  <div
                    key={idx}
              ...
              <div className="flex justify-end mt-6">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-italian-purple text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-purple/20 transition-all duration-200"
                >
                  Continue to {imperfettoStages[STAGE_ORDER.indexOf(activeStage) + 1]?.title} <ArrowRight size={16} />
                </button>
              </div>
            </motion.div>
          )}
                ...
                <div className="space-y-6">
                          ...
                          className={`w-9 h-9 shrink-0 rounded-lg text-xs font-bold transition-all duration-200 flex items-center justify-center ${
                            i === currentExercise
                              ...
                              : results[e.id]?.correct
                                ? 'bg-italian-purple/10 text-italian-purple border border-italian-purple/20'
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-gray-400 dark:text-gray-500">
                      Exercise {currentEx.id} / {imperfettoExercises.length}
                    </span>
                    <button
                      onClick={() => goToExercise(currentExercise + 1)}
                      disabled={currentExercise === imperfettoExercises.length - 1}
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-italian-purple text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-purple/20 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
                    >
