...
import { modaliStages, modaliExercises, modaliFinalBands } from './modaliData';
import {
...
const STORAGE_KEY = 'modaliResults';
  ...
  const renderExamples = (examples) => (
    <div className="space-y-2">
      {examples.map((ex, i) => (
        ...
        >
          <span className="text-sm font-semibold text-italian-teal whitespace-nowrap">
            {ex.italian}
          </span>
        ...
        <tbody>
          {conv.rows.map((row, i) => (
            <tr key={i} className="border-t border-gray-100 dark:border-italian-dark-border">
              ...
              <td className="px-4 py-2.5 font-semibold text-italian-teal">{row.ending}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  const renderTimeline = (tl) => (
    <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
      <div className="flex items-center justify-between text-xs font-bold text-gray-400 dark:text-gray-500 mb-3">
        <span>{tl.left}</span>
        <span className="text-italian-teal">{tl.center}</span>
        <span>{tl.right}</span>
      </div>
      <div className="relative h-1 bg-gray-200 dark:bg-italian-dark-border rounded-full mb-4">
        ...
        <div className="absolute left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-italian-teal ring-2 ring-italian-teal/30" />
          ...
          {tl.examples.map((ex, i) => (
            <span
              ...
              className={`inline-flex px-3 py-1.5 rounded-lg text-xs font-semibold border ${
                i === 1
                  ? 'bg-italian-teal/10 text-italian-teal border-italian-teal/20'
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        {focus.breakdown.map((b, i) => (
          ...
          >
            <p className="text-base font-bold text-italian-teal">{b.part}</p>
      ...
      {section.patterns && (
        <div className="flex flex-wrap gap-2">
          {section.patterns.map((p, i) => (
            <span
              key={i}
              className="inline-flex px-3 py-1.5 rounded-lg bg-italian-teal/10 text-italian-teal text-sm font-bold border border-italian-teal/20"
            >
              {p}
            </span>
          ))}
...
function ReviewSection() {
  return (
    <div className="space-y-6">
      <h3 className="text-base font-heading font-bold text-italian-charcoal dark:text-white">Final Review — Modal Verbs Reference</h3>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-teal uppercase tracking-wider mb-3">The Key Idea</h4>
        <p className="text-lg font-semibold text-italian-charcoal dark:text-white">
          Modal verbs = <span className="text-italian-teal">dovere, potere, volere</span> — obligation, possibility, and desire — always followed by an <span className="text-italian-teal">infinitive</span>.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-teal uppercase tracking-wider mb-3">Dovere — present</h4>
          <div className="space-y-1.5">
            {['io devo', 'tu devi', 'lui/lei deve', 'noi dobbiamo', 'voi dovete', 'loro devono'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
          <p className="text-xs text-gray-400 mt-2">obligation: Devo studiare.</p>
        </div>
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-teal uppercase tracking-wider mb-3">Potere — present</h4>
          <div className="space-y-1.5">
            {['io posso', 'tu puoi', 'lui/lei può', 'noi possiamo', 'voi potete', 'loro possono'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
          <p className="text-xs text-gray-400 mt-2">ability/permission: Posso? = May I?</p>
        </div>
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-teal uppercase tracking-wider mb-3">Volere — present</h4>
          <div className="space-y-1.5">
            {['io voglio', 'tu vuoi', 'lui/lei vuole', 'noi vogliamo', 'voi volete', 'loro vogliono'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
          <p className="text-xs text-gray-400 mt-2">desire: Voglio un caffè.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-teal uppercase tracking-wider mb-3">Passato prossimo</h4>
          <div className="space-y-1.5">
            {['ho dovuto + infinitive', 'ho potuto + infinitive', 'ho voluto + infinitive', 'Lei ha dovuto pagare.', 'Non ho potuto venire.'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
          <p className="text-xs text-gray-400 mt-2">The main verb stays an infinitive.</p>
        </div>
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-teal uppercase tracking-wider mb-3">Imperfetto</h4>
          <div className="space-y-1.5">
            {['dovevo / potevo / volevo', 'dovevamo / potevamo / volevamo', 'Da piccolo dovevo andare presto.', 'Potevo uscire quando volevo.'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
          <p className="text-xs text-gray-400 mt-2">Repeated or ongoing actions in the past.</p>
        </div>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-teal uppercase tracking-wider mb-3">Futuro &amp; Condizionale Stems</h4>
        <div className="flex flex-wrap gap-2">
          {['dovere → dovrò / dovrei', 'potere → potrò / potrei', 'volere → vorrò / vorrei', 'Domani dovrò lavorare.', 'Potrei aiutarti, se vuoi.', 'Vorrei un caffè, per favore.', 'Dovresti studiare di più.'].map((p) => (
            <span key={p} className="inline-flex px-3 py-1.5 rounded-lg bg-italian-teal/10 text-italian-teal text-xs font-bold border border-italian-teal/20">{p}</span>
          ))}
        </div>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-teal uppercase tracking-wider mb-3">Polite Everyday Phrases</h4>
        <div className="flex flex-wrap gap-2">
          {['Posso? (May I?)', 'Vorrei un caffè.', 'Potrebbe aiutarmi? (formal)', 'Potresti chiudere la porta?', 'Dovresti riposare.', 'Non devi fumare qui.', 'Devo andare.'].map((p) => (
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-teal uppercase tracking-wider mb-3">Golden Rule</h4>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          The <span className="font-semibold text-italian-charcoal dark:text-white">modal verb is conjugated</span> and the <span className="font-semibold text-italian-charcoal dark:text-white">main verb follows as an infinitive</span> — never conjugated. Negation goes before the modal (<span className="font-semibold text-italian-teal">non posso</span>), and the condizionale forms (<span className="font-semibold text-italian-teal">vorrei, potrei, dovrei, potrebbe, potresti</span>) are the polite way to request, ask permission, and give advice.
        </p>
      </div>
    </div>
  );
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="px-2.5 py-1 rounded-lg bg-italian-teal/10 text-italian-teal text-xs font-bold border border-italian-teal/20">
          ...
          {isDone && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-italian-teal/10 text-italian-teal text-xs font-bold">
...
function ResultsSummary({ results, onReset }) {
  ...
  const reviewMap = {
    'Level 1': 'Review the three modal verbs (dovere, potere, volere) and the rule that they are followed by an infinitive.',
    'Level 2': 'Review the present tense: devo/devi/deve/dobbiamo/dovete/devono, posso/puoi/può/possiamo/potete/possono, voglio/vuoi/vuole/vogliamo/volete/vogliono.',
    'Level 3': 'Practice choosing the right modal: obligation (dovere), ability/permission (potere), desire (volere).',
    'Level 4': 'Review the structure: conjugated modal + infinitive, with negation and question word order.',
    'Level 5': 'Review the shades of meaning: devi (must/should), puoi (can/may), vorrei (polite), dovrei (advice), and dovere for logical deduction.',
    'Level 6': 'Review the past: ho dovuto/potuto/voluto + infinitive (passato prossimo) vs dovevo/potevo/volevo (imperfetto, habits).',
    'Level 7': 'Review futurato and condizionale stems: dovrò/dovrei, potrò/potrei, vorrò/vorrei and their polite uses.',
    'Level 8': 'Practice negation, questions, and choosing the right tense from time expressions (ieri/domani/da piccolo).',
    'Level 9': 'Re-read the dialogues — notice how posso, devi, vorrei, dovresti, and potrebbe are used in real conversations.',
  };
  ...
  return (
    <div className="space-y-6">
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="rounded-2xl border border-italian-teal/30 bg-italian-teal/5 p-6 text-center"
      >
        <Trophy className="w-10 h-10 text-italian-teal mx-auto mb-2" />
        <p className="text-3xl font-heading font-bold text-italian-teal">{accuracy}%</p>
          ...
          <div className="space-y-3">
            {Object.entries(weakLevels).map(([lvl, count]) => (
              <div key={lvl} className="flex items-start gap-3">
                <span className="px-2 py-0.5 rounded-md bg-italian-teal/10 text-italian-teal text-xs font-bold border border-italian-teal/20 shrink-0 mt-0.5">
                  {lvl}
                </span>
                <div>
...
const allExercisesDone = (results) =>
  modaliExercises.every((e) => results[e.id]?.correct);
...
const STORY_EXERCISE_START = modaliExercises.findIndex((e) => e.type === 'story');

export default function ModaliLessonPage() {
  ...
  const completedCount = useMemo(
    () => modaliExercises.filter((e) => results[e.id]?.correct).length,
    [results]
  );
  ...
  const exerciseStart = useMemo(() => ({
    ...
    final: modaliExercises.length - 1,
  }), []);

  const currentEx = modaliExercises[currentExercise];
  const isFinalStage = activeStage === 'final';
  const showSummary = isFinalStage && allExercisesDone(results);

  const goToExercise = (idx) => {
    const clamped = Math.max(0, Math.min(modaliExercises.length - 1, idx));
    setCurrentExercise(clamped);
    const ex = modaliExercises[clamped];
  ...
  const handleStageClick = (stage) => {
    ...
    if (stage === 'final') setCurrentExercise(modaliExercises.length - 1);
  };

  const globalProgress = Math.round((completedCount / modaliExercises.length) * 100);
    ...
    <div className="min-h-full pb-24 md:pb-8">
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mb-6">
          <Link to="/grammar" className="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-italian-teal transition-colors mb-4">
            <ArrowLeft size={16} /> Back to Grammar
          </Link>
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-italian-teal/10 flex items-center justify-center">
              <BookOpen size={22} className="text-italian-teal" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="px-2 py-0.5 rounded-md text-xs font-bold border bg-italian-teal/15 text-italian-teal border-italian-teal/30">
                  A2–B1
                </span>
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                Dovere, Potere &amp; Volere — obligation, ability, and desire in every tense
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="flex-1 h-1.5 bg-gray-100 dark:bg-italian-dark-surface rounded-full overflow-hidden">
              <motion.div
                ...
                className="h-full bg-italian-teal rounded-full"
              />
            </div>
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 shrink-0">
              {completedCount}/{modaliExercises.length} exercises
            </span>
          </div>
        </motion.div>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0">
          {[
            { id: 'concept', icon: BookOpen, label: '1. What are Modal Verbs?' },
            { id: 'formation', icon: Layers, label: '2. Conjugating the Modals' },
            { id: 'irregulars', icon: Grid3x3, label: '3. Other Tenses' },
            { id: 'usage', icon: Type, label: '4. Using the Modals' },
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 shrink-0 ${
                activeStage === id
                  ? 'bg-italian-teal text-white shadow-md shadow-italian-teal/20'
          ...
          {LEARNING_STAGES.includes(activeStage) && (
            ...
            >
              <h2 className="text-lg font-heading font-bold text-italian-charcoal dark:text-white mb-1">
                {modaliStages[STAGE_ORDER.indexOf(activeStage)]?.title}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                {modaliStages[STAGE_ORDER.indexOf(activeStage)]?.subtitle}
              </p>
              <div className="space-y-6">
                {modaliStages[STAGE_ORDER.indexOf(activeStage)]?.sections.map((section, idx) => (
                  <div
                    key={idx}
              ...
              <div className="flex justify-end mt-6">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-italian-teal text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-teal/20 transition-all duration-200"
                >
                  Continue to {modaliStages[STAGE_ORDER.indexOf(activeStage) + 1]?.title} <ArrowRight size={16} />
                </button>
              </div>
            </motion.div>
          )}
                ...
                <div className="space-y-6">
                          ...
                          className={`w-9 h-9 shrink-0 rounded-lg text-xs font-bold transition-all duration-200 flex items-center justify-center ${
                            i === currentExercise
                              ...
                              : results[e.id]?.correct
                                ? 'bg-italian-teal/10 text-italian-teal border border-italian-teal/20'
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-gray-400 dark:text-gray-500">
                      Exercise {currentEx.id} / {modaliExercises.length}
                    </span>
                    <button
                      onClick={() => goToExercise(currentExercise + 1)}
                      disabled={currentExercise === modaliExercises.length - 1}
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-italian-teal text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-teal/20 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
                    >
