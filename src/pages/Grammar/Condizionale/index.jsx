...
import { condizionaleStages, condizionaleExercises, condizionaleFinalBands } from './condizionaleData';
import {
...
const STORAGE_KEY = 'condizionaleResults';
...
function ReviewSection() {
  return (
    <div className="space-y-6">
      <h3 className="text-base font-heading font-bold text-italian-charcoal dark:text-white">Final Review — Condizionale Reference</h3>
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        ...
        <p className="text-lg font-semibold text-italian-charcoal dark:text-white">
          Condizionale = <span className="text-italian-gold">hypothetical situations, wishes, polite requests, and advice</span> — the Italian "would".
        </p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-gold uppercase tracking-wider mb-3">Regular Endings</h4>
        <div className="flex flex-wrap gap-2">
          {['-are → -erei: parlare → parlerei', '-ere → -erei: prendere → prenderei', '-ire → -irei: partire → partirei', 'io -ei · tu -esti · lui -ebbe · noi -emmo · voi -este · loro -ebbero'].map((p) => (
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {['andare → andr- → andrei', 'avere → avr- → avrei', 'essere → sar- → sarei', 'fare → far- → farei', 'venire → verr- → verrei', 'volere → vorr- → vorrei', 'potere → potr- → potrei', 'dovere → dovr- → dovrei', 'sapere → sapr- → saprei', 'vedere → vedr- → vedrei', 'bere → berr- → berrei', 'rimanere → rimarr- → rimarrei'].map((p) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-green uppercase tracking-wider mb-3">Tipo 1 — Real</h4>
          <div className="space-y-1.5">
            {['Se piove, prendo l\'ombrello.', 'Se studi, passi l\'esame.', 'Se domani fa bel tempo, usciamo.'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
          <p className="text-xs text-gray-400 mt-2">SE + presente → presente/futuro</p>
        </div>
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-green uppercase tracking-wider mb-3">Tipo 2 — Hypothetical</h4>
          <div className="space-y-1.5">
            {['Se avessi tempo, andrei al mare.', 'Se fossi ricco, comprerei una casa.', 'Se fossi in te, parlerei con lui.'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
          <p className="text-xs text-gray-400 mt-2">SE + congiuntivo imperfetto → condizionale presente</p>
        </div>
        <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
          <h4 className="text-sm font-bold text-italian-green uppercase tracking-wider mb-3">Tipo 3 — Impossible Past</h4>
          <div className="space-y-1.5">
            {['Se avessi studiato, avrei passato.', 'Se fossi partito prima, non avrei perso il treno.', 'Saremmo usciti, se non avesse piovuto.'].map((f) => (
              <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
            ))}
          </div>
          <p className="text-xs text-gray-400 mt-2">SE + congiuntivo trapassato → condizionale passato</p>
        </div>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-gold uppercase tracking-wider mb-3">Polite Everyday Phrases</h4>
        <div className="flex flex-wrap gap-2">
          {['Vorrei un caffè.', 'Potrebbe aiutarmi?', 'Potresti chiudere la porta?', 'Mi piacerebbe visitare Roma.', 'Dovremmo parlare.', 'Se fossi in te, non lo farei.'].map((p) => (
            <span key={p} className="inline-flex px-3 py-1.5 rounded-lg bg-italian-gold/10 text-italian-gold text-xs font-bold border border-italian-gold/20">{p}</span>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-gold uppercase tracking-wider mb-3">The Condizionale Passato</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-italian-gold/5 border border-italian-gold/20">
            <p className="text-xs font-bold text-italian-gold uppercase tracking-wider mb-2">avrei + participle</p>
            <div className="space-y-1">
              {['avrei mangiato', 'avresti parlato', 'avremmo visto'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">sarei + participle (agrees)</p>
            <div className="space-y-1">
              {['sarei andato/a', 'saremmo partiti', 'sarebbero usciti'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
        </div>
        <p className="text-xs text-gray-400 mt-3">Use avrei with transitive verbs and most -are/-ire verbs; sarei with motion/reflexive verbs. The participle agrees with sarei.</p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-gold uppercase tracking-wider mb-3">Golden Rule</h4>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          The <span className="font-semibold text-italian-charcoal dark:text-white">condizionale never appears inside the se-clause</span>, and the future is never used there either. In tipo 2 and 3 the se-clause uses the congiuntivo. Only the main (result) clause takes the condizionale.
        </p>
      </div>
    </div>
  );
...
function ResultsSummary({ results, onReset }) {
  ...
  const total = condizionaleExercises.length;
  const accuracy = Math.round((completed / total) * 100);

  const bands = condizionaleFinalBands;
  const band = bands.find((b) => accuracy >= b.min) || bands[bands.length - 1];

  const weakLevels = {};
  condizionaleExercises.forEach((e) => {
  ...
  const reviewMap = {
    'Level 1': 'Review what the periodo ipotetico is and the difference between real, hypothetical, and impossible conditions (tipo 1, 2, 3).',
    'Level 2': 'Review the regular condizionale endings: -ei, -esti, -ebbe, -emmo, -este, -ebbero, and the -ARE → -ER- change.',
    'Level 3': 'Review essere (sar-), avere (avr-), the common irregular stems, and the congiuntivo in se-clauses.',
    'Level 4': 'Review the condizionale passato: avrei/sarei + past participle and when the participle must agree.',
    'Level 5': 'Review tipo 1 (se + presente → presente/futuro) and the rule: never use the future in the se-clause.',
    'Level 6': 'Review tipo 2 (se + congiuntivo imperfetto + condizionale presente) and polite requests with vorrei/potresti.',
    'Level 7': 'Review tipo 3 (se + congiuntivo trapassato + condizionale passato) for expressing regrets about the past.',
    'Level 8': 'Practice building mixed sentences, choosing the right forms, and translating natural conditional sentences.',
    'Level 9': 'Re-read the dialogues — pay attention to the se-clause tenses and the result tenses in each type.',
  };
...
const allExercisesDone = (results) =>
  condizionaleExercises.every((e) => results[e.id]?.correct);
...
const STORY_EXERCISE_START = condizionaleExercises.findIndex((e) => e.type === 'story');

export default function CondizionaleLessonPage() {
  ...
  const completedCount = useMemo(
    () => condizionaleExercises.filter((e) => results[e.id]?.correct).length,
    [results]
  );
  ...
  const exerciseStart = useMemo(() => ({
    ...
    final: condizionaleExercises.length - 1,
  }), []);

  const currentEx = condizionaleExercises[currentExercise];
  const isFinalStage = activeStage === 'final';
  const showSummary = isFinalStage && allExercisesDone(results);

  const goToExercise = (idx) => {
    const clamped = Math.max(0, Math.min(condizionaleExercises.length - 1, idx));
    setCurrentExercise(clamped);
    const ex = condizionaleExercises[clamped];
  ...
  const handleStageClick = (stage) => {
    ...
    if (stage === 'final') setCurrentExercise(condizionaleExercises.length - 1);
  };

  const globalProgress = Math.round((completedCount / condizionaleExercises.length) * 100);
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-italian-sage/10 flex items-center justify-center">
              <BookOpen size={22} className="text-italian-sage" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                Condizionale — present & past, plus all three types of the periodo ipotetico
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="flex-1 h-1.5 bg-gray-100 dark:bg-italian-dark-surface rounded-full overflow-hidden">
              <motion.div
                ...
                className="h-full bg-italian-sage rounded-full"
              />
            </div>
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 shrink-0">
              {completedCount}/{condizionaleExercises.length} exercises
            </span>
          </div>
        </motion.div>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0">
          {[
            { id: 'concept', icon: BookOpen, label: '1. What is the Condizionale?' },
            { id: 'formation', icon: Layers, label: '2. Forming the Condizionale' },
            { id: 'irregulars', icon: Grid3x3, label: '3. Essere, Avere & Irregulars' },
            { id: 'usage', icon: Type, label: '4. The Three Types' },
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 shrink-0 ${
                activeStage === id
                  ? 'bg-italian-sage text-white shadow-md shadow-italian-sage/20'
          ...
          {LEARNING_STAGES.includes(activeStage) && (
            ...
            >
              <h2 className="text-lg font-heading font-bold text-italian-charcoal dark:text-white mb-1">
                {condizionaleStages[STAGE_ORDER.indexOf(activeStage)]?.title}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                {condizionaleStages[STAGE_ORDER.indexOf(activeStage)]?.subtitle}
              </p>
              <div className="space-y-6">
                {condizionaleStages[STAGE_ORDER.indexOf(activeStage)]?.sections.map((section, idx) => (
                  <div
                    key={idx}
              ...
              <div className="flex justify-end mt-6">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-italian-sage text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-sage/20 transition-all duration-200"
                >
                  Continue to {condizionaleStages[STAGE_ORDER.indexOf(activeStage) + 1]?.title} <ArrowRight size={16} />
                </button>
              </div>
            </motion.div>
          )}
                ...
                <div className="space-y-6">
                          ...
                          className={`w-9 h-9 shrink-0 rounded-lg text-xs font-bold transition-all duration-200 flex items-center justify-center ${
                            i === currentExercise
                              ...
                              : results[e.id]?.correct
                                ? 'bg-italian-sage/10 text-italian-sage border border-italian-sage/20'
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-gray-400 dark:text-gray-500">
                      Exercise {currentEx.id} / {condizionaleExercises.length}
                    </span>
                    <button
                      onClick={() => goToExercise(currentExercise + 1)}
                      disabled={currentExercise === condizionaleExercises.length - 1}
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-italian-sage text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-sage/20 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
                    >
