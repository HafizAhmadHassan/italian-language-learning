...
import { pronomiRelativiStages, pronomiRelativiExercises, pronomiRelativiFinalBands } from './pronomiRelativiData';
import {
...
const STORAGE_KEY = 'pronomiRelativiResults';
...
function SectionLearner({ section }) {
  ...
  const renderExamples = (examples) => (
    <div className="space-y-2">
      {examples.map((ex, i) => (
        ...
        >
          <span className="text-sm font-semibold text-italian-cyan whitespace-nowrap">
            {ex.italian}
          </span>
  ...
  const renderVerbs = (section) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      {section.verbs.map((v, i) => (
        ...
        >
          <span className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{v.infinitive}</span>
        ...
        <tbody>
          {conv.rows.map((row, i) => (
            <tr key={i} className="border-t border-gray-100 dark:border-italian-dark-border">
              ...
              <td className="px-4 py-2.5 font-semibold text-italian-cyan">{row.ending}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  const renderTimeline = (tl) => (
    <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
      <div className="flex items-center justify-between text-xs font-bold text-gray-400 dark:text-gray-500 mb-3">
        <span>{tl.left}</span>
        <span className="text-italian-cyan">{tl.center}</span>
        <span>{tl.right}</span>
      </div>
      <div className="relative h-1 bg-gray-200 dark:bg-italian-dark-border rounded-full mb-4">
        ...
        <div className="absolute left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-italian-cyan ring-2 ring-italian-cyan/30" />
          ...
          {tl.examples.map((ex, i) => (
            <span
              ...
              className={`inline-flex px-3 py-1.5 rounded-lg text-xs font-semibold border ${
                i === 1
                  ? 'bg-italian-cyan/10 text-italian-cyan border-italian-cyan/20'
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        {focus.breakdown.map((b, i) => (
          ...
          >
            <p className="text-base font-bold text-italian-cyan">{b.part}</p>
  ...
  return (
    <div className="space-y-4">
      ...
      {section.patterns && (
        <div className="flex flex-wrap gap-2">
          {section.patterns.map((p, i) => (
            <span
              key={i}
              className="inline-flex px-3 py-1.5 rounded-lg bg-italian-cyan/10 text-italian-cyan text-sm font-bold border border-italian-cyan/20"
            >
              {p}
            </span>
          ))}
      ...
      {section.note && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-italian-cyan/10 border border-italian-cyan/25">
          <MessageSquare size={16} className="text-italian-cyan mt-0.5 shrink-0" />
...
function ReviewSection() {
  return (
    <div className="space-y-6">
      <h3 className="text-base font-heading font-bold text-italian-charcoal dark:text-white">Final Review — Relative Pronouns Reference</h3>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-cyan uppercase tracking-wider mb-3">The Key Idea</h4>
        <p className="text-lg font-semibold text-italian-charcoal dark:text-white">
          A relative pronoun <span className="text-italian-cyan">connects two clauses about the same noun</span> — it replaces the antecedent instead of repeating it. Your four tools: <span className="text-italian-cyan">che, cui, il quale, chi</span>.
        </p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-cyan uppercase tracking-wider mb-3">The Four Pronouns</h4>
        <div className="flex flex-wrap gap-2">
          {['che → subject or direct object (invariable)', 'cui → after a preposition (di cui, in cui, a cui…)', 'il quale / la quale / i quali / le quali → agrees, formal', 'chi → "the one who", no antecedent', 'il cui / la cui / i cui / le cui → "whose"'].map((p) => (
            <span key={p} className="inline-flex px-3 py-1.5 rounded-lg bg-italian-cyan/10 text-italian-cyan text-xs font-bold border border-italian-cyan/20">{p}</span>
          ))}
        </div>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-cyan uppercase tracking-wider mb-3">Cui with Prepositions</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-italian-cyan/5 border border-italian-cyan/20">
            <p className="text-xs font-bold text-italian-cyan uppercase tracking-wider mb-2">Common pairs</p>
            <div className="space-y-1">
              {['di cui — about / of which (parlare di)', 'a cui — to whom (scrivere a)', 'in cui — in which (abitare in)', 'con cui — with whom (uscire con)'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">More pairs</p>
            <div className="space-y-1">
              {['per cui — for which / that is why (lavorare per)', 'da cui — from whom (venire da)', 'su cui — on which', 'a cui — to whom (pensare a)'].map((u) => (
                <p key={u} className="text-sm font-medium text-gray-700 dark:text-gray-300">{u}</p>
              ))}
            </div>
          </div>
        </div>
        <p className="text-xs text-gray-400 mt-3">Learn each verb with its preposition: che is never combined with a preposition.</p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-cyan uppercase tracking-wider mb-3">The Possessive il cui ("whose")</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Singular</p>
            <div className="space-y-1">
              {['il cui + m sing → la donna il cui nome…', 'la cui + f sing → la ragazza la cui sorella…'].map((f) => (
                <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
              ))}
            </div>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Plural</p>
            <div className="space-y-1">
              {['i cui + m pl → i bambini i cui libri…', 'le cui + f pl → le città le cui strade…'].map((f) => (
                <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
              ))}
            </div>
          </div>
        </div>
        <p className="text-xs text-gray-400 mt-3">Agreement follows the POSSESSED noun (the one after cui), never the possessor.</p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-cyan uppercase tracking-wider mb-3">chi — the free relative</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-italian-cyan/5 border border-italian-cyan/20">
            <p className="text-xs font-bold text-italian-cyan uppercase tracking-wider mb-2">No antecedent — "the one who"</p>
            <div className="space-y-1">
              {['Chi cerca, trova.', 'Chi non lavora, non mangia.', 'Chi vuole, può.'].map((f) => (
                <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
              ))}
            </div>
          </div>
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-italian-dark-surface border border-gray-100 dark:border-italian-dark-border">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Indirect questions</p>
            <div className="space-y-1">
              {['Non so chi sia.', 'Non ricordo chi me l\'ha detto.', 'Dimmi chi hai visto!'].map((f) => (
                <p key={f} className="text-sm font-semibold text-italian-charcoal dark:text-gray-100">{f}</p>
              ))}
            </div>
          </div>
        </div>
        <p className="text-xs text-gray-400 mt-3">chi always takes a third-person singular verb and needs no previous noun.</p>
      </div>

      <div className="rounded-2xl border border-gray-100 dark:border-italian-dark-border bg-white dark:bg-italian-dark-card p-5">
        <h4 className="text-sm font-bold text-italian-cyan uppercase tracking-wider mb-3">Golden Rule</h4>
        <p className="text-sm text-gray-600 dark:text-gray-400">
          <span className="font-semibold text-italian-charcoal dark:text-white">No preposition</span> → use <span className="font-semibold text-italian-charcoal dark:text-white">che</span>. <span className="font-semibold text-italian-charcoal dark:text-white">A preposition</span> → preposition + <span className="font-semibold text-italian-charcoal dark:text-white">cui</span> or <span className="font-semibold text-italian-charcoal dark:text-white">il quale</span>. <span className="font-semibold text-italian-charcoal dark:text-white">Possession</span> → <span className="font-semibold text-italian-charcoal dark:text-white">il cui / la cui / i cui / le cui</span>. <span className="font-semibold text-italian-charcoal dark:text-white">No antecedent and "the one who"</span> → <span className="font-semibold text-italian-charcoal dark:text-white">chi</span>. Never: che after a preposition.
        </p>
      </div>
    </div>
  );
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="px-2.5 py-1 rounded-lg bg-italian-cyan/10 text-italian-cyan text-xs font-bold border border-italian-cyan/20">
          ...
          {isDone && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-italian-cyan/10 text-italian-cyan text-xs font-bold">
...
function ResultsSummary({ results, onReset }) {
  ...
  const total = pronomiRelativiExercises.length;
  const accuracy = Math.round((completed / total) * 100);

  const bands = pronomiRelativiFinalBands;
  const band = bands.find((b) => accuracy >= b.min) || bands[bands.length - 1];

  const weakLevels = {};
  pronomiRelativiExercises.forEach((e) => {
  ...
  const reviewMap = {
    'Level 1': 'Review what relative pronouns do: they connect two clauses, replacing the antecedent (che, cui, il quale, chi).',
    'Level 2': 'Review che as subject and as direct object — it never takes a preposition.',
    'Level 3': 'Review il quale / la quale / i quali / le quali and their agreement with the noun.',
    'Level 4': 'Review cui with the correct preposition: in cui, di cui, a cui, con cui, per cui, da cui.',
    'Level 5': 'Review the possessive il cui / la cui / i cui / le cui (whose) — it agrees with the possessed noun that follows.',
    'Level 6': 'Review chi ("the one who") used without an antecedent and in indirect questions.',
    'Level 7': 'Practice choosing among che, cui, il quale, and chi for each sentence.',
    'Level 8': 'Practice complex everyday sentences, including idiomatic "quello che" and proverbs with chi.',
    'Level 9': 'Re-read the dialogues — notice every relative pronoun and the noun (or preposition) it connects to.',
  };
  ...
  return (
    <div className="space-y-6">
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="rounded-2xl border border-italian-cyan/30 bg-italian-cyan/5 p-6 text-center"
      >
        <Trophy className="w-10 h-10 text-italian-cyan mx-auto mb-2" />
        <p className="text-3xl font-heading font-bold text-italian-cyan">{accuracy}%</p>
          ...
          <div className="space-y-3">
            {Object.entries(weakLevels).map(([lvl, count]) => (
              <div key={lvl} className="flex items-start gap-3">
                <span className="px-2 py-0.5 rounded-md bg-italian-cyan/10 text-italian-cyan text-xs font-bold border border-italian-cyan/20 shrink-0 mt-0.5">
                  {lvl}
                </span>
                <div>
...
const allExercisesDone = (results) =>
  pronomiRelativiExercises.every((e) => results[e.id]?.correct);
...
const STORY_EXERCISE_START = pronomiRelativiExercises.findIndex((e) => e.type === 'story');

export default function PronomiRelativiLessonPage() {
  ...
  const completedCount = useMemo(
    () => pronomiRelativiExercises.filter((e) => results[e.id]?.correct).length,
    [results]
  );
  ...
  const exerciseStart = useMemo(() => ({
    ...
    final: pronomiRelativiExercises.length - 1,
  }), []);

  const currentEx = pronomiRelativiExercises[currentExercise];
  const isFinalStage = activeStage === 'final';
  const showSummary = isFinalStage && allExercisesDone(results);

  const goToExercise = (idx) => {
    const clamped = Math.max(0, Math.min(pronomiRelativiExercises.length - 1, idx));
    setCurrentExercise(clamped);
    const ex = pronomiRelativiExercises[clamped];
  ...
  const handleStageClick = (stage) => {
    ...
    if (stage === 'final') setCurrentExercise(pronomiRelativiExercises.length - 1);
  };

  const globalProgress = Math.round((completedCount / pronomiRelativiExercises.length) * 100);
    ...
    <div className="min-h-full pb-24 md:pb-8">
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="mb-6">
          <Link to="/grammar" className="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-italian-cyan transition-colors mb-4">
            <ArrowLeft size={16} /> Back to Grammar
          </Link>
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-italian-cyan/10 flex items-center justify-center">
              <BookOpen size={22} className="text-italian-cyan" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="px-2 py-0.5 rounded-md text-xs font-bold border bg-italian-cyan/15 text-italian-cyan border-italian-cyan/30">
                  A2–B1
                </span>
              </div>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                che, cui, il quale, chi — connect your sentences naturally
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="flex-1 h-1.5 bg-gray-100 dark:bg-italian-dark-surface rounded-full overflow-hidden">
              <motion.div
                ...
                className="h-full bg-italian-cyan rounded-full"
              />
            </div>
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 shrink-0">
              {completedCount}/{pronomiRelativiExercises.length} exercises
            </span>
          </div>
        </motion.div>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0">
          {[
            { id: 'concept', icon: BookOpen, label: '1. What are Relative Pronouns?' },
            { id: 'formation', icon: Layers, label: '2. Che & Il Quale' },
            { id: 'irregulars', icon: Grid3x3, label: '3. Cui, Chi & Special Uses' },
            { id: 'usage', icon: Type, label: '4. Choosing the Right Pronoun' },
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 shrink-0 ${
                activeStage === id
                  ? 'bg-italian-cyan text-white shadow-md shadow-italian-cyan/20'
          ...
          {LEARNING_STAGES.includes(activeStage) && (
            ...
            >
              <h2 className="text-lg font-heading font-bold text-italian-charcoal dark:text-white mb-1">
                {pronomiRelativiStages[STAGE_ORDER.indexOf(activeStage)]?.title}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                {pronomiRelativiStages[STAGE_ORDER.indexOf(activeStage)]?.subtitle}
              </p>
              <div className="space-y-6">
                {pronomiRelativiStages[STAGE_ORDER.indexOf(activeStage)]?.sections.map((section, idx) => (
                  <div
                    key={idx}
              ...
              <div className="flex justify-end mt-6">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-italian-cyan text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-cyan/20 transition-all duration-200"
                >
                  Continue to {pronomiRelativiStages[STAGE_ORDER.indexOf(activeStage) + 1]?.title} <ArrowRight size={16} />
                </button>
              </div>
            </motion.div>
          )}
              ...
              {activeStage === 'final' && showSummary ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1">
                    {pronomiRelativiExercises.map((e, i) => {
                          ...
                          className={`w-9 h-9 shrink-0 rounded-lg text-xs font-bold transition-all duration-200 flex items-center justify-center ${
                            i === currentExercise
                              ...
                              : results[e.id]?.correct
                                ? 'bg-italian-cyan/10 text-italian-cyan border border-italian-cyan/20'
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-semibold text-gray-400 dark:text-gray-500">
                      Exercise {currentEx.id} / {pronomiRelativiExercises.length}
                    </span>
                    <button
                      onClick={() => goToExercise(currentExercise + 1)}
                      disabled={currentExercise === pronomiRelativiExercises.length - 1}
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-italian-cyan text-white text-sm font-semibold hover:shadow-lg hover:shadow-italian-cyan/20 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
                    >
