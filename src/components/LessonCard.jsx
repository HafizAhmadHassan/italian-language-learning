...
export default function LessonCard({ lesson, isCompleted, isLocked, progress = 0 }) {
  ...
  return (
    <MotionLink
      to={isLocked ? undefined : `/lesson/${lesson.id}`}
      ...
      className={`block rounded-2xl border p-5 transition-all duration-200 ${
        isLocked
          ? 'bg-gray-50 border-gray-200 opacity-50 cursor-not-allowed dark:bg-italian-dark-card dark:border-italian-dark-border pointer-events-none'
